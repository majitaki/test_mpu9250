...
*/
void FaBo9Axis::configMPU9250(int gfs, int afs)
{
	switch (gfs)
	{
...
*/
void FaBo9Axis::configAK8963(int mode, int mfs)
{
	int data[3];
...
*/
void FaBo9Axis::writeI2c(int address, int register_addr, int data)
{
	int fd;
	if ((fd = wiringPiI2CSetup(address)) == -1)
	{
		Debug::print(LOG_SUMMARY, "Failed to Setup and Write I2C device\r\n");
		return;
	}
...
*/
void FaBo9Axis::readI2c(int address, int register_addr, int num, int * buffer)
{
	int fd;

	if ((fd = wiringPiI2CSetup(address)) == -1)
	{
		Debug::print(LOG_SUMMARY, "Failed to Setup and Read I2C device\r\n");
		return;
	}
	...
	else
	{
		for (int i = 0; i < num; i++)
		{
			int start_addr = register_addr + i;
			int end_addr = start_addr + 1;
			unsigned tmp = (wiringPiI2CReadReg8(fd, start_addr) << 8) | wiringPiI2CReadReg8(fd, end_addr);
			buffer[i] = tmp;
			register_addr += 1;
		}
		return;
